...
# Analysis object: /MC_JETS/jet_eta_pmratio_4
...
ax_xLabel = r'$|\eta(\mathrm{jet~4}|$'
ax_yLabel = r'$\eta(\mathrm{jet~4})_+/\eta(\mathrm{jet~4})_-$'
ax_title  = r'Pseudorapidity $+/-$ ratio of fourth jet'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (0.0, 5.0)
yLims = (0.11, 1.0)
...
exec(open(prefix+'jet_eta_pmratio_4__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'jet_eta_pmratio_4.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'jet_eta_pmratio_4.png'), format='PNG')
