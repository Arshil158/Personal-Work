...
# Analysis object: /MC_JETS/jet_HT
...
ax_yLabel = r'$\mathrm{d}\sigma/\mathrm{d}H_\mathrm{T}$ [pb/GeV]'
ax_title  = r'Scalar sum of jet transverse momenta ($H_\mathrm{T}$)'
ax_xScale = 'log'
ax_yScale = 'linear'
xLims = (20.0, 6500.0)
yLims = (0.11, 1.0)
...
exec(open(prefix+'jet_HT__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'jet_HT.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'jet_HT.png'), format='PNG')
