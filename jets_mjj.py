...
# Analysis object: /MC_JETS/jets_mjj
...
ax_yLabel = r'$\mathrm{d}\sigma/\mathrm{d}m_{jj}$ [pb/GeV]'
ax_title  = r'Dijet invariant mass spectrum'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.0, 6500.0)
yLims = (0.11, 1.0)
...
exec(open(prefix+'jets_mjj__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'jets_mjj.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'jets_mjj.png'), format='PNG')
