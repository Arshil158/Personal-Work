...
# Analysis object: /MC_JETS/jet_multi_exclusive
...
ax_yLabel = r'$\sigma(N_{\mathrm{jet}})$ [pb]'
ax_title  = r'Exclusive jet multiplicity'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (-0.5, 6.5)
...
exec(open(prefix+'jet_multi_exclusive__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'jet_multi_exclusive.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'jet_multi_exclusive.png'), format='PNG')
