...
# Analysis object: /MC_JETS/jet_eta_1
...
ax_yLabel = r'$\mathrm{d}\sigma/\mathrm{d}\eta(\mathrm{jet~1})$ [pb]'
ax_title  = r'Pseudorapidity of leading jet'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (-5.0, 5.0)
yLims = (0.11, 1.0)
...
exec(open(prefix+'jet_eta_1__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'jet_eta_1.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'jet_eta_1.png'), format='PNG')
