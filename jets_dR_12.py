...
# Analysis object: /MC_JETS/jets_dR_12
...
ax_xLabel = r'$\Delta{R}(\mathrm{jet~1,~jet~2})$'
ax_yLabel = r'$\mathrm{d}\sigma/\mathrm{d}\Delta{R}(\mathrm{jet~1,~jet~2})$ [pb]'
ax_title  = r'$\Delta R$ separation between jets'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.0, 5.0)
yLims = (0.11, 1.0)
...
exec(open(prefix+'jets_dR_12__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'jets_dR_12.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'jets_dR_12.png'), format='PNG')
