...
# Analysis object: /MC_JETS/jet_eta_pmratio_3
...
ax_xLabel = r'$|\eta(\mathrm{jet~3}|$'
ax_yLabel = r'$\eta(\mathrm{jet~3})_+/\eta(\mathrm{jet~3})_-$'
ax_title  = r'Pseudorapidity $+/-$ ratio of third jet'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (0.0, 5.0)
yLims = (0.11, 1.0)
...
exec(open(prefix+'jet_eta_pmratio_3__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'jet_eta_pmratio_3.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'jet_eta_pmratio_3.png'), format='PNG')
